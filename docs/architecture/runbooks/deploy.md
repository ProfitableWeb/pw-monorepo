...
1. **CI** (`ci.yml` как reusable workflow): ruff + pytest, lint + type-check + test фронтендов. **Красный CI → деплой
   `skipped`**, на сервер ничего не уходит (PW-081)
...
Обхода гейта нет намеренно — именно так в прод уезжали сломанные тесты и типы (PW-080). Варианты:
...
Dev живёт в **отдельном каталоге** `~/profitableweb-dev` (до PW-081 prod и dev делили `~/profitableweb`, и dev-деплой
