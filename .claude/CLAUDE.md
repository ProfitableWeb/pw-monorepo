...
`author/` (динамическая карточка автора из API), `categories/` (список категорий), `my-comments/` (комментарии
пользователя), `preview/` (превью статей для админки). Генерируемые: `feed.xml/`, `sitemap.xml/`, `robots.ts`.
