import React from 'react';

export interface SocialLink {
  name: string;
  href: string;
  icon: React.ReactNode;
}

// Все доступные социальные сети
const ALL_SOCIAL_LINKS: SocialLink[] = [
  {
    name: 'VKontakte',
    href: 'https://vk.com/profitableweb.ru',
    icon: (
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M22.9946 5.88952C23.1531 5.37774 22.9946 5 22.2509 5H19.7881C19.1663 5 18.8737 5.329 18.7152 5.68237C18.7152 5.68237 17.4594 8.67994 15.6915 10.6296C15.1185 11.1901 14.8625 11.3729 14.5455 11.3729C14.387 11.3729 14.1553 11.1901 14.1553 10.6783V5.88952C14.1553 5.28026 13.9846 5 13.4604 5H9.59547C9.20532 5 8.97367 5.28026 8.97367 5.56052C8.97367 6.14541 9.86369 6.27945 9.94904 7.91227V11.4703C9.94904 12.2502 9.80273 12.3964 9.49793 12.3964C8.66886 12.3964 6.63277 9.38668 5.42575 5.93826C5.1941 5.26807 4.95026 5 4.32846 5H1.84126C1.13411 5 1 5.329 1 5.68237C1 6.32819 1.82907 9.49634 4.8893 13.7002C6.92538 16.5638 9.79054 18.1235 12.3997 18.1235C13.9603 18.1235 14.1553 17.7823 14.1553 17.1852V15.0162C14.1553 14.3217 14.3016 14.1877 14.8015 14.1877C15.1673 14.1877 15.7891 14.3704 17.2521 15.7474C18.9225 17.3924 19.2029 18.1235 20.1417 18.1235H22.6045C23.3116 18.1235 23.6652 17.7823 23.4579 17.0999C23.2385 16.4175 22.4338 15.4305 21.3853 14.2608C20.8122 13.5906 19.9466 12.8716 19.6906 12.5183C19.3248 12.0552 19.4345 11.8481 19.6906 11.446C19.6906 11.446 22.6898 7.29082 23.0068 5.88952H22.9946Z'
        fill='currentColor'
      />
    ),
  },
  {
    name: 'Telegram',
    href: 'https://t.me/ProfitableWeb_ru',
    icon: (
      <path
        d='M20.9439 5.45585L18.0725 19.0357C17.8538 19.9969 17.2932 20.2303 16.4864 19.7772L12.1247 16.5504L10.019 18.5826C9.78658 18.816 9.59516 19.0082 9.14395 19.0082L9.45843 14.5457L17.553 7.19968C17.9085 6.88386 17.4709 6.70536 17.006 7.02117L6.99726 13.3511L2.6902 11.9918C1.74675 11.7034 1.73308 11.0443 2.88163 10.6049L19.7407 4.08276C20.52 3.79442 21.2037 4.26127 20.9439 5.45585Z'
        fill='currentColor'
      />
    ),
  },
  {
    name: 'Yandex Dzen',
    href: 'https://dzen.ru/profitableWeb_ru',
    icon: (
      <path
        d='M13.5402 13.5402C12.2794 14.84 12.1885 16.4517 12.0845 20C15.3859 20 17.6734 20 18.8432 18.8432C19.987 17.6604 20 15.2819 20 12.0845C16.4517 12.2015 14.84 12.2794 13.5402 13.5402ZM4 12.0845C4 15.2819 4 17.6734 5.15678 18.8432C6.33956 19.987 8.61413 20 11.9155 20C11.7985 16.4517 11.7206 14.84 10.4598 13.5402C9.16003 12.2794 7.54833 12.1885 4 12.0845ZM11.9155 4C8.61413 4 6.33956 4 5.15678 5.15678C4.013 6.33956 4 8.71812 4 11.9155C7.54833 11.7985 9.16003 11.7206 10.4598 10.4598C11.7206 9.16003 11.8115 7.54833 11.9155 4ZM13.5402 10.4598C12.2924 9.16003 12.2015 7.54833 12.0845 4C15.3859 4 17.6734 4 18.8432 5.15678C19.987 6.33956 20 8.71812 20 11.9155C16.4517 11.7985 14.84 11.7206 13.5402 10.4598Z'
        fill='currentColor'
      />
    ),
  },
  {
    name: 'GitHub',
    href: 'https://github.com/profitableweb', // TODO: Add actual GitHub link
    icon: (
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z'
        fill='currentColor'
      />
    ),
  },
];

// Вспомогательная функция для получения ссылок по именам
const getLinksByName = (names: string[]): SocialLink[] => {
  return ALL_SOCIAL_LINKS.filter(link => names.includes(link.name));
};

/**
 * Конфигурация социальных сетей для футера
 * Все доступные сети
 */
export const SOCIAL_LINKS_FOOTER: SocialLink[] = getLinksByName([
  'VKontakte',
  'Telegram',
  'Yandex Dzen',
]);

/**
 * Конфигурация социальных сетей для блока автора
 * VK, Telegram, Yandex Dzen, GitHub
 */
export const SOCIAL_LINKS_AUTHOR: SocialLink[] = getLinksByName([
  'VKontakte',
  'Telegram',
  'Yandex Dzen',
  'GitHub',
]);

/**
 * Конфигурация социальных сетей для app-bar
 * VK, Telegram, Yandex Dzen
 */
export const SOCIAL_LINKS_APPBAR: SocialLink[] = getLinksByName([
  'VKontakte',
  'Telegram',
  'Yandex Dzen',
]);

/**
 * Дефолтная конфигурация (для обратной совместимости)
 * @deprecated Используйте конкретные конфигурации: SOCIAL_LINKS_FOOTER, SOCIAL_LINKS_AUTHOR, SOCIAL_LINKS_APPBAR
 */
export const SOCIAL_LINKS: SocialLink[] = ALL_SOCIAL_LINKS;
