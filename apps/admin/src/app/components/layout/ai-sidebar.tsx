...
const recentChats: Chat[] = [
  {
    ...
    preview:
      'Предложи темы для статьи про современные тренды в веб-дизайне 2024...',
  },
  {
    id: '2',
      ...
      <div>
        ...
        <div className='space-y-2'>
          {autonomousProcesses.map(process => {
            ...
            return (
              <div key={process.id} className='p-3 rounded-lg border bg-card'>
...
export function AiSidebar({ isOpen, isMobile, onClose }: AiSidebarProps) {
  // На мобильных — без обёртки aside
  if (isMobile) {
    return (
  ...
  // Десктопная версия — статичная, без перетаскивания
  return (
    <aside
      className={cn(
