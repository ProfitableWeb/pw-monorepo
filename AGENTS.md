...
Репозиторий **публичный** (GitHub + GitVerse). Секрет в истории компрометируется навсегда — нужна ротация, не только
чистка HEAD.

- Никогда не коммитить реальные значения: пароли, токены, `Bearer`/`pw_mcp_*`, приватные ключи, OAuth secret, `.env`
  (кроме `*.env.example` с плейсхолдерами), дампы сессий AI с окружением.
- В compose для секретов — fail-fast `${VAR:?required}`, не `${VAR:-дефолт}`. В доках — `<плейсхолдер>`, не реальные ID.
- Pre-commit сканер `scripts/check-secrets.mjs` (через lint-staged) блокирует коммит; ложное срабатывание — маркер
  `secret-scan:allow` в конце строки.
- Канонический файл: `.cursor/rules/security-secrets.mdc`.
